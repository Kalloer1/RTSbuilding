...
public final class RtsWorkflowSyncService {
    ...
    public void notifyPlayer(ServerPlayer player, RtsWorkflowSlotManager slots) {
        ...
        if (totalCount == 0) {
            RtsPacketSender.sendToPlayer(player, S2CRtsWorkflowProgressPayload.idle());
            return;
        }
        ...
        RtsPacketSender.sendToPlayer(player, new S2CRtsWorkflowProgressBatchPayload(entries));
    }
    ...
    public void notifyCompletion(ServerPlayer player, RtsWorkflowSlotManager slots,
        ...
        RtsPacketSender.sendToPlayer(player, new S2CRtsWorkflowProgressPayload(
                (byte) removedAtIndex,
                (byte) remainingCount,
                status.type() != null ? (byte) status.type().ordinal() : (byte) -1,
                (byte) status.priority().rank(),
